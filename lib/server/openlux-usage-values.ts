...
export function parseTokenUsage(value: unknown): TokenUsage {
    ...
    if (Object.keys(gemini).length) {
        ...
        result.outputTokens = candidates === null ? null : candidates + (result.reasoningTokens ?? 0);
        result.totalTokens = count(gemini.totalTokenCount);
        if (Array.isArray(gemini.promptTokensDetails)) {
            const images = gemini.promptTokensDetails.map(obj).filter(item => String(item.modality).toUpperCase() === 'IMAGE');
            const counts = images.map(item => count(item.tokenCount));
            if (counts.every(n => n !== null)) result.imageInputTokens = counts.reduce<number>((sum, n) => sum + (n ?? 0), 0);
        }
    } else {
